...
class SendConfig extends Command
{
    ...
    {
        //获取默认配置文件
        $content = file_get_contents(__DIR__. '/../addons/config.php');
        ...
        if (is_file($configFile)) {
            throw new Exception(sprintf('The config file "%s" already exists', $configFile));
        }

        if (false === file_put_contents($configFile, $content)) {
            throw new Exception(sprintf('The config file "%s" could not be written to "%s"', $configFile,$configPath));
        }
