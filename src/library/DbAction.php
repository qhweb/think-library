...
class DbAction{
  ...
  {
    return $this->app->config->get('database.connections.mysql.prefix');
  }
  //数据库自动类型
  public function getCharset()
  {
    return $this->app->config->get('database.connections.mysql.charset');
  }
  ...
  public function deleteTable($table = '')
  {
    ...
    return !empty($table) ? $this->app->db->execute("DROP TABLE IF EXISTS `{$tableName}`") : false;
  }
  /**
   * 创建数据表
   */
  public function createTable($table='',$title='')
  {
    try {
      $tableName = $this->getPrefix().$table;
      $this->app->db->execute("CREATE TABLE `".$tableName."` (
   ...
   * 修改数据表名
   */
  public function updateTable($oldtable='',$table='')
  {
    $sql="ALTER TABLE ".$this->getPrefix() .$oldtable." RENAME TO ".$this->getPrefix() .$table;
   ...
   * 插入字段
   */
	public function createField($data,$table){
    //sql字段值定义
    ...
    $statusSql = $this->app->db->execute($sql);
		return $statusSql;
	}
   ...
   * 修改字段
   */
	public function updateField($oldfiled,$data,$table){
...
// e($property);
  ...
  public function deleteField($table,$field){
    $tableName = $this->getPrefix().$table;
    return $this->app->db->execute("ALTER TABLE {$tableName} DROP COLUMN {$field}");
  }
  /**
   * 检查数据表字段是否存在
   */
  ...
  {
    $tableFields = $this->app->db->getTableFields($this->getPrefix() . $table);
 ...
 {
   if(!$table) return [];
   return  $this->app->db->getFields($table);
 }
 /**
  * 模型数据格式化
    ...
    if($fieldInfo['hidden']){
      ...
      switch($fieldInfo['type']){
          ...
          $multiple = 'one';//one或者btn的时候单个文件，其他多个文件
  				...
  				$str .="						<label class=\"layui-form-label\">".$fieldInfo['name']."</label>\n";
          $str .="						<div class=\"layui-input-block\">\n";
          $str .="							<textarea id=\"".$fieldInfo['field']."\" allownum=\"{$fieldInfo['setup']['upload_allownum']}\" name=\"".$fieldInfo['field']."\" class=\"layui-hide\" data-type=\"".$upload_allowext."\">".$defaultValue."</textarea>\n";
  				//多图上传
          if($fieldInfo['setup']['upload_type'] == 'kindeditor'){
            $str .="              <script>$('#".$fieldInfo['field']."').kindEditorUploadMultiple();</script>\n";
          }else{
            if($fieldInfo['setup']['upload_allownum'] > 1){
              $str .="              <script>$('#".$fieldInfo['field']."').uploadMultipleImage();</script>\n";
            }else{
              $str .="              <script>$('#".$fieldInfo['field']."').uploadOneImage();</script>\n";
            }
          }
          ...
          $multiple = 'one';//one或者btn的时候单个文件，其他多个文件
  				...
  				$str .="						<label class=\"layui-form-label\">".$fieldInfo['name']."</label>\n";
          $str .="						<div class=\"layui-input-block\">\n";
          $str .="							<textarea allownum=\"{$fieldInfo['setup']['upload_allownum']}\" id=\"".$fieldInfo['field']."\" name=\"".$fieldInfo['field']."\" class=\"layui-hide\"  data-type=\"".$upload_allowext."\">".$defaultValue."</textarea>\n";
          $str .="							<script>$('#".$fieldInfo['field']."').uploadMultipleFile();</script>\n";
